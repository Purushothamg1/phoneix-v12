...
const PDF_DIR = path_1.default.join(UPLOAD_DIR, 'pdfs');
...
async function generateQrDataUrl(text) {
    try {
        return await qrcode_1.default.toDataURL(text, { width: 80, margin: 1, color: { dark: '#1e293b', light: '#ffffff' } });
    }
    catch {
        return null;
    }
...
function drawHeader(doc, settings, logoPath) {
    ...
    if (settings.business_address)
        doc.text(settings.business_address, 50, 72);
    if (settings.business_phone)
        doc.text(`Phone: ${settings.business_phone}`);
    if (settings.business_email)
        doc.text(`Email: ${settings.business_email}`);
    if (settings.gst_number)
        doc.text(`GST: ${settings.gst_number}`);
}
...
exports.pdfService = {
    ...
    async generateInvoicePdf(invoiceId) {
        ...
        return new Promise((resolve, reject) => {
            const doc = new pdfkit_1.default({ size: 'A4', margin: 50, info: {
            ...
            if (invoice.customer.email)
                doc.text(invoice.customer.email, 50, 186);
            if (invoice.customer.address)
                doc.text(invoice.customer.address, 50, invoice.customer.email ? 199 : 186);
            // ── Items Table ────────────────────────────────────────
            const tableTop = 235;
            ...
            doc.text(`${currency}${subtotal.toFixed(2)}`, 465, y, { width: 75, align: 'right' });
            y += 15;
            doc.text('Tax:', 380, y);
            doc.text(`${currency}${Number(invoice.taxAmount).toFixed(2)}`, 465, y, { width: 75, align: 'right' });
            y += 15;
            ...
            doc.end();
            stream.on('finish', () => {
                logger_1.logger.info(`PDF generated: ${filename}`);
                resolve(`/uploads/pdfs/${filename}`);
            });
            stream.on('error', (err) => {
                logger_1.logger.error(`PDF generation failed: ${filename}`, err);
                reject(err);
            });
        });
    },
     ...
     */
    async generateRepairPdf(repairId) {
        ...
        const currency = settings.currency_symbol || '₹';
        const appUrl = process.env.APP_URL || process.env.FRONTEND_URL || '';
        const qrDataUrl = await generateQrDataUrl(appUrl ? `${appUrl}/repairs/${repair.id}` : `Repair: ${repair.jobId} | ${repair.brand} ${repair.model}`);
        return new Promise((resolve, reject) => {
            const doc = new pdfkit_1.default({ size: 'A4', margin: 50, info: {
            ...
            if (repair.parts.length > 0) {
                ...
                doc.text('Part Name', 55, y + 5);
                doc.text('SKU', 250, y + 5);
                doc.text('Qty', 340, y + 5);
                doc.text('Unit Cost', 390, y + 5);
                doc.text('Subtotal', 465, y + 5);
                y += 22;
                let partsTotal = 0;
     ...
     */
    async regenerateRepairPdf(repairId) {
